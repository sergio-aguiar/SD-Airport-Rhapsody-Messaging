    ...
    public void prepareForNextFlight() {
        ...
        while (!clientCom.open()) {
            try {
                ...
                System.out.println("Thread " + Thread.currentThread().getName() + ": ATTQStub: prepareForNextFlight: " + e.toString());
            }
        }
        ...
        if(inMessage.getMessageType() != Message.MessageType.ATTQ_PREPARE_FOR_NEXT_FLIGHT.getMessageCode()) {
            GenericIO.writelnString("Thread " + Thread.currentThread().getName() + ": ATTQStub: prepareForNextFlight: Incorrect message type!");
            GenericIO.writelnString(inMessage.toString());
            System.exit(1);
        }
        clientCom.close();
