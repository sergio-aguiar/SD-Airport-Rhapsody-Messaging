    ...
    public void prepareForNextFlight() {
        ...
        while (!clientCom.open()) {
            try {
                ...
                System.out.println("Thread " + Thread.currentThread().getName() + ": BROStub: prepareForNextFlight: " + e.toString());
            }
        }

        try {
            outMessage = new Message(Message.MessageType.BRO_PREPARE_FOR_NEXT_FLIGHT.getMessageCode(), false, true);
        } catch(MessageException e) {
            System.out.println("Thread " + Thread.currentThread().getName() + ": BROStub: prepareForNextFlight: " + e.toString());
        }
        ...
        if(inMessage.getMessageType() != Message.MessageType.BRO_PREPARE_FOR_NEXT_FLIGHT.getMessageCode()) {
            GenericIO.writelnString("Thread " + Thread.currentThread().getName() + ": BROStub: prepareForNextFlight: Incorrect message type!");
            GenericIO.writelnString(inMessage.toString());
            System.exit(1);
        }
        clientCom.close();
