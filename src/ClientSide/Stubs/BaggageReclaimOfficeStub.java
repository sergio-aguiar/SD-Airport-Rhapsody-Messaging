    ...
    public void reportMissingBags(int pid, int missingBags) {
        ...
        while (!clientCom.open()) {
            try {
                ...
                System.out.println("Thread " + Thread.currentThread().getName() + ": BROQStub: reportMissingBags: " + e.toString());
            }
        }
        ...
        if(inMessage.getMessageType() != Message.MessageType.PA_BRO_REPORT_MISSING_BAGS.getMessageCode()) {
            GenericIO.writelnString("Thread " + Thread.currentThread().getName() + ": BROQStub: reportMissingBags: Incorrect message type!");
