    ...
    public void everythingFinished() {
        ...
        while (!clientCom.open()) {
            try {
                ...
                System.out.println("Thread " + Thread.currentThread().getName() + ": BROStub: everythingFinished: " + e.toString());
            }
        }
        ...
        if(inMessage.getMessageType() != Message.MessageType.EVERYTHING_FINISHED.getMessageCode()) {
            GenericIO.writelnString("Thread " + Thread.currentThread().getName() + ": BROStub: everythingFinished: Incorrect message type!");
            GenericIO.writelnString(inMessage.toString());
            System.exit(1);
        }
        clientCom.close();
